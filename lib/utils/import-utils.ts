...
export async function parseExcel<TData>(
  ...
  try {
    ...
    if (!sheetName) {
      return {
        data: [],
        errors: [{ row: 0, message: "Le fichier Excel ne contient aucune feuille" }],
        success: false,
      }
    }
    ...
    if (!worksheet) {
      return {
        data: [],
        errors: [{ row: 0, message: "Impossible de lire la feuille Excel" }],
        success: false,
      }
    }
      ...
      errors: [
        {
          ...
          message:
            error instanceof Error
              ? error.message
              : "Échec de l'analyse du fichier Excel",
        },
      ],
      success: false,
    }
    ...
    requiredFields.forEach((field) => {
      if (!row[field]) {
        errors.push({
          row: index + 1,
          field: String(field),
          message: `Champ requis manquant: ${String(field)}`,
        })
      }
    })
  })
