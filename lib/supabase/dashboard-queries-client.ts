/**
 * Dashboard Queries - Client Side
 *
 * Client-side queries for dashboard statistics (for use in hooks)
 */
...
export async function getDashboardStats(
  ...
  const { data: containerData } = await supabase
    ...
    .gte("trajet.date_trajet", formatDateForQuery(dateFrom))
    .lte("trajet.date_trajet", formatDateForQuery(dateTo));
...
export async function getCostChartData(
  ...
  tripData?.forEach((trip) => {
    ...
    const totalCost =
      fuelCost + (trip.frais_peage || 0) + (trip.autres_frais || 0);
